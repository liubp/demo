(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope, $translate) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('simpleApp.httpError', function (event, httpResponse) {
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-simpleApp-error');
                var entityKey = httpResponse.headers('X-simpleApp-params');
                if (errorHeader) {
                    ...
                    for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
                        ...
                        var fieldName = $translate.instant('simpleApp.' + fieldError.objectName + '.' + convertedField);
